...
#
#	Original version:
#	http://zuttobenkyou.wordpress.com/2009/03/26/adding-replay-gain-in-linux-automatically/
#	
...
class Ape2Id3(object):
   def __init__(self, logger, force=False, id3v23=False):
       self.log = logger
       self.force = force
       self.id3v23 = id3v23
   ...
   def copy_replaygain_tags(self, filename):
       ...
       if modified:
           if self.id3v23:
               self.log.debug("saving modified ID3 tag as ID3v2.3")
               id3.update_to_v23()
               id3.save(filename, v2=3)
           else:
               self.log.debug("saving modified ID3 tag as ID3v2.4")
               id3.save(filename)
...
def main(prog_name, options, args):
   ...
   ape2id3 = Ape2Id3(logger, force=options.force, id3v23=options.id3v23)
...
if __name__ == "__main__":
   ...
   parser.add_option("-f", "--force", dest="force",
                     ...
                     help="force overwriting of existing ID3v2 "
                          "ReplayGain tags")
   parser.add_option("-3", "--use-id3v2.3", dest="id3v23",
                     action="store_true", default=False,
                     help="write ID3v2.3 tags instead of ID3v2.4")
